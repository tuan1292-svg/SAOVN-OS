...
function render() {
  const search=$('searchInput'),status=$('statusFilter'),role=$('roleFilter'); if(!search||!status||!role)return; const q=search.value.trim().toLowerCase(),sf=status.value,rf=role.value;
  ...
  document.querySelectorAll('.member-row').forEach(row=>{const open=()=>{const member=members.find(m=>m.id===row.dataset.memberId);if(!member||member.status!=='ACTIVE')return;openMemberDetail(member);};if(row.dataset.memberId&&row.getAttribute('aria-disabled')!=='true'){row.addEventListener('click',open);row.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();open();}});}});
}
