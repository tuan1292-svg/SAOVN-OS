...
export async function getWorkScope(){
  ...
  const [identitySnap,membershipSnap]=await Promise.all([getDoc(doc(db,'identities',user.uid)),getDoc(doc(db,'memberships',MEMBERSHIP_ID(user.uid)))]);
...
export function taskInScope(task,scope){
  ...
  const embeddedAssignees=Array.isArray(task?.assignees)?task.assignees.map(a=>clean(a?.id)).filter(Boolean):[];
  if(scope.uid===createdBy || assignedIds.includes(scope.uid) || embeddedAssignees.includes(scope.uid) || legacyAssignee===scope.uid) return true;
