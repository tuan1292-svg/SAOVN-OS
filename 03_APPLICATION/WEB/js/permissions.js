...
/* Roles are authorization roles, not job titles. CEO/CFO/CTO/etc. remain titles and do not silently become MANAGER. */
function normalizeRole(value){const role=String(value||'').trim().toUpperCase().replace(/[\s-]+/g,'_');if(!role)return'MEMBER';if(['SYSTEM_ADMIN','ADMINISTRATOR','ORG_ADMIN','OWNER','SUPER_ADMIN','ADMIN'].includes(role))return'ADMIN';if(['MANAGER','TEAM_LEAD','DEPARTMENT_MANAGER'].includes(role))return'MANAGER';return'MEMBER';}
...
function buildExperienceRuntime(user,membership,policy){const effective=effectiveMembership(membership,state.role);const context=buildRuntimeContext({user,membership:effective,policy});const person=toPerson(user,effective);return Object.freeze({...context,person,rawRoleIds:rawRoleIds(effective),position:person.position,title:person.title});}
...
export function can(area,action='read'){
  const aliases={'members.read':PERMISSIONS.MEMBERS_VIEW,'members.manage':PERMISSIONS.MEMBERS_UPDATE,'members.create':PERMISSIONS.MEMBERS_CREATE,'members.update':PERMISSIONS.MEMBERS_UPDATE,'members.delete':PERMISSIONS.MEMBERS_DELETE,'members.role.manage':PERMISSIONS.MEMBERS_ROLE_MANAGE,'departments.read':PERMISSIONS.DEPARTMENTS_VIEW,'departments.manage':PERMISSIONS.DEPARTMENTS_MANAGE,'dashboard.read':PERMISSIONS.DASHBOARD_VIEW,'work.view':PERMISSIONS.WORK_VIEW,'work.read':PERMISSIONS.WORK_VIEW,'work.create':PERMISSIONS.WORK_CREATE,'work.edit':PERMISSIONS.WORK_EDIT,'work.delete':PERMISSIONS.WORK_DELETE,'work.assign':PERMISSIONS.WORK_ASSIGN,'work.comment':PERMISSIONS.WORK_COMMENT,'work.checklist':PERMISSIONS.WORK_CHECKLIST,'attendance.read':PERMISSIONS.ATTENDANCE_VIEW,'attendance.manage':PERMISSIONS.ATTENDANCE_MANAGE};
  const key=`${area}.${action}`; return hasPermission(key)||Boolean(aliases[key]&&hasPermission(aliases[key]));
}
...
function applyNavigation(){const route=currentRoute();document.querySelectorAll('a[href]').forEach(node=>{const raw=String(node.getAttribute('href')||'').split('#')[0].split('?')[0];const target=raw.split('/').pop()?.toLowerCase();const moduleId=ROUTE_MODULES.get(target);if(!moduleId)return;const allowed=routeAllowed(target,moduleId);node.hidden=!allowed;node.setAttribute('aria-hidden',String(!allowed));});document.querySelectorAll('[data-capability]').forEach(node=>{const capability=node.dataset.capability;if(!capability)return;const allowed=hasPermission(capability);node.hidden=!allowed;node.setAttribute('aria-hidden',String(!allowed));});const controlPlaneAllowed=hasPermission(PERMISSIONS.SYSTEM_MANAGE);ensureControlPlaneEntry(controlPlaneAllowed);document.querySelectorAll('[data-admin-navigation="true"]').forEach(node=>{node.hidden=!controlPlaneAllowed;});if(route==='admin-control.html'&&!controlPlaneAllowed){window.location.replace('dashboard.html');return;}if(route!=='dashboard.html'&&ROUTE_MODULES.has(route)&&!routeAllowed(route,ROUTE_MODULES.get(route)))window.location.replace('dashboard.html');}
async function load(user){if(!user)return state;activeUser=user;state.uid=user.uid;try{const snap=await getDoc(doc(db,'memberships',MEMBERSHIP_ID(user.uid)));activeMembership=snap.exists()?snap.data():{};}catch(error){activeMembership={};console.warn('Membership unavailable; using safe MEMBER baseline.',error?.code||error);}const role=roleFromMembership(activeMembership);const policy=await loadPolicy();activePolicy=policy;const effective=effectiveMembership(activeMembership,role);const person=toPerson(user,effective);const context=Object.freeze({...buildRuntimeContext({user,membership:effective,policy}),person,rawRoleIds:rawRoleIds(effective),position:person.position,title:person.title});state={ready:true,uid:user.uid,role,permissions:context.capabilities,context,person:context.person};window.SAOVNRuntime=context;applyNavigation();watchPolicy();window.dispatchEvent(new CustomEvent('saovn:permissions-ready',{detail:state}));window.dispatchEvent(new CustomEvent('saovn:runtime-ready',{detail:context}));return state;}
