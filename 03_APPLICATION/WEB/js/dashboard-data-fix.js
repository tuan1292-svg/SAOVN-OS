...
function render(tasks){
 const today=dateKey(new Date()),total=tasks.length,done=tasks.filter(t=>t.status==="DONE").length,active=tasks.filter(t=>t.status==="IN_PROGRESS").length,waiting=tasks.filter(t=>["BACKLOG","TODO","REVIEW"].includes(t.status)).length,overdue=tasks.filter(t=>toDateKey(t.dueDate)&&toDateKey(t.dueDate)<today&&t.status!=="DONE").length,score=total?Math.round(tasks.reduce((a,t)=>a+progress(t),0)/total):0;
 const cards=document.querySelectorAll(".metric-card");
 if(cards[0]){cards[0].querySelector(".metric-head span").textContent="TỔNG CÔNG VIỆC";cards[0].querySelector("strong").textContent=total;cards[0].querySelector("small").textContent="Theo dữ liệu Work hiện tại";}
 ...
 const p=$(".report-card .report-copy p:not(.eyebrow)");if(p)p.textContent=`Đang theo dõi ${total} công việc từ Work. Tiến độ hiện tại ${score}%, ${done} đã hoàn thành và ${overdue} quá hạn.`;
 const ul=$(".report-card .report-copy ul");if(ul)ul.innerHTML=`<li>${total} công việc trong phạm vi tài khoản.</li><li>${active} công việc đang thực hiện.</li><li>${waiting} công việc chờ xử lý/review.</li><li>${overdue} công việc quá hạn.</li>`;
