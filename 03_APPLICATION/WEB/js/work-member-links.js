...
async function loadPeople() {
  try {
    ...
    try {
      ...
      console.warn('Work member links: ACTIVE identities query failed; loading identity directory fallback.', error?.code || error);
    ...
    identitySnap.docs.forEach(s => {
      ...
      const person = {
        ...
        identity: d,
        membership: m,
        position: labels[String(m.position || d.position || d.jobTitle || '').toUpperCase()] || m.position || d.position || d.jobTitle || 'Nhân viên',
...
function enhanceAssigneeText(root) {
  ...
  root.querySelectorAll('.assignee, .kanban-assignee').forEach(el => {
    ...
    el.innerHTML = parts.map(part => linkByName(part.split(' · ')[0].trim())).join(', ');
...
function enhanceDetail(root) {
  ...
  first.innerHTML = names.map(name => linkByName(name)).join(', ');
...
function ensureWorkNavigation() {
  const sidebar = document.querySelector('.sidebar');
  if (!sidebar || !document.querySelector('.nav-group')) return;
  const groups = [...sidebar.querySelectorAll('.nav-group')];
  const core = groups[0];
  if (core && !core.querySelector('a[href="departments.html"]')) {
    const memberLink = core.querySelector('a[href="members.html"]');
    const a = document.createElement('a');
    a.className = 'nav-item';
    a.href = 'departments.html';
    a.innerHTML = '▤ <span>Phòng ban</span>';
    if (memberLink) core.insertBefore(a, memberLink); else core.appendChild(a);
  }
  let exchange = groups.find(g => g.dataset.navigationExchange === '1');
  if (!exchange) {
    exchange = document.createElement('div');
    exchange.className = 'nav-group';
    exchange.dataset.navigationExchange = '1';
    exchange.innerHTML = '<span class="nav-title">TRAO ĐỔI</span><a class="nav-item" href="chat.html">▢ <span>Trò chuyện</span></a><a class="nav-item" href="notifications.html">♧ <span>Thông báo</span></a>';
    const moduleGroup = groups.find(g => g.querySelector('a[href="work.html"].module'));
    sidebar.insertBefore(exchange, moduleGroup || sidebar.querySelector('.sidebar-bottom'));
  }
  sidebar.querySelectorAll('a.nav-item').forEach(a => {
    const href = a.getAttribute('href') || '';
    if (href === 'work.html') a.classList.add('active');
    else if (/^(dashboard|departments|members|chat|notifications)\.html/.test(href)) a.classList.remove('active');
  });
}
...
loadPeople().then(() => { ensureWorkNavigation(); observe(); });