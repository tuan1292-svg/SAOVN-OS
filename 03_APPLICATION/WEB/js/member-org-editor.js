...
function setEditorEnabled(enabled){['positionSelect','departmentInput','teamInput','roleSelect','phoneInput'].forEach(id=>{const el=$(id);if(el)el.disabled=!enabled});const button=$('saveMemberBtn');if(button)button.hidden=!enabled}
...
async function populateEditor(identityId){
  ...
  try{
    ...
    const position=membership.position||identity.position||'STAFF',departmentId=membership.departmentId||identity.departmentId||'',legacyDepartment=membership.department||identity.department||'',phone=identity.phone||identity.phoneNumber||identity.mobile||'',email=identity.email||identity.emailAddress||'',team=membership.team||identity.team||'';
    ...
    if($('detailTeam'))$('detailTeam').textContent=team||'Chưa phân nhóm';
...
async function saveOrganizationFields(){
  if(!currentIsAdmin){status('Chỉ Admin mới được điều chỉnh vị trí, phòng ban, Team và liên hệ.','error');return}
  const identityId=$('detailId')?.textContent?.trim();
  if(!identityId||identityId==='—'){status('Không xác định được thành viên.','error');return}
  const position=$('positionSelect')?.value||'STAFF',departmentId=$('departmentInput')?.value||'',department=departments.find(d=>d.id===departmentId)?.name||'',team=$('teamInput')?.value?.trim()||'',role=$('roleSelect')?.value||'MEMBER',phone=$('phoneInput')?.value?.trim()||'';
  ...
  try{
    ...
    await setDoc(identityRef,{position,departmentId:departmentId||null,department,team,phone,updatedAt:serverTimestamp(),updatedBy:currentUser.uid},{merge:true});
    await setDoc(membershipRef,{position,departmentId:departmentId||null,department,team,roles:nextRoles,updatedAt:serverTimestamp(),updatedBy:currentUser.uid},{merge:true});
    ...
    status('Đã lưu vị trí, phòng ban, Team và thông tin liên hệ.','success');
