...
onAuthStateChanged(auth,async user=>{
  ...
  try{
    ...
    let members=[];
    try{const ids=await getDocs(collection(db,'identities'));members=ids.docs.map(s=>({id:s.id,...s.data()}));}catch(error){members=[{id:user.uid,...identity}];}
    ...
    const rows=members.map(member=>{const assigned=tasks.filter(t=>Array.isArray(t.assigneeIds)?t.assigneeIds.includes(member.id):t.assigneeId===member.id);const done=assigned.filter(t=>t.status==='DONE');const today=new Date().toISOString().slice(0,10);const overdue=assigned.filter(t=>t.dueDate&&t.dueDate<today&&t.status!=='DONE');const onTime=done.filter(t=>!t.dueDate||taskTime(t)<=new Date(`${t.dueDate}T23:59:59`).getTime()).length;const percent=assigned.length?Math.round(done.length/assigned.length*100):0;return {member,total:assigned.length,done:done.length,overdue,onTime,percent};}).filter(r=>r.total>0||r.member.id===user.uid).sort((a,b)=>b.percent-a.percent||b.total-a.total);
